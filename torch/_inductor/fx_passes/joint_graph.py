    ...
    with torch.utils._python_dispatch._disable_current_modes():
        ...
        def replace_no_op(node, replace_input_index):
            ...
            if not all(isinstance(arg, torch.fx.Node) for arg in node.args):
                if all(isScalarValue(arg) for arg in node.args) or not isinstance(replacement, torch.fx.Node):
                    return
        ...
        for node in graph.find_nodes(op="call_function", target=aten.add.Tensor):
            if len(node.args) == 2:
                if (
                    not any(e in zeros or (isScalarValue(e) and e == 0) for e in node.args)
